...
inline __device__ void compute_and_sum_pix_velocity(
    const float3 p_view,
    ...
    float3 &v_p_view_accumulator)
{
    ...
    glm::vec3 rot_part = glm::cross(ang_vel, p_view_glm);
    float iz_reg = 1.0 / (p_view.z + 1e-6f);
    glm::vec3 total_vel = lin_vel + rot_part;

    glm::vec3 v_total_vel_npc = {
        -f_x * v_pix_velocity.x,
        -f_y * v_pix_velocity.y,
        0.f
    };
...
inline __device__ float3 project_pix_vjp(
    const float f_x, const float f_y, const float3 p_view, const float2 v_xy
) {
    float rw = 1.f / (p_view.z + 1e-6f);
    float2 v_proj = { f_x * v_xy.x, f_y * v_xy.y };
    float3 v_view = {
...
__global__ void computeCov2DCUDA(int P,
    ...
    float3 p_world = means[idx];
    float3 t = transformPoint4x3(p_world, view_matrix);

    float3 v_p_view_pix_vel = { 0, 0, 0 };
    if (exposure_time > -1.f) {
        const float2 v_pix_velocity = dL_pixel_vels[idx];
        compute_and_sum_pix_velocity(t, *linear_velocity, *angular_velocity, f_x, f_y, v_pix_velocity, v_p_view_pix_vel);
    }

    float3 v_p_view = project_pix_vjp(f_x, f_y, t, {dL_dmean2D[idx].x, dL_dmean2D[idx].y});
    v_p_view.x += v_p_view_pix_vel.x;
    v_p_view.y += v_p_view_pix_vel.y;

    v_p_view.z += v_p_view_pix_vel.z;
    dL_dmeans[idx] = transform_4x3_rot_only_transposed(view_matrix, v_p_view);
...
__global__ void __launch_bounds__(BLOCK_X* BLOCK_Y)
    renderCUDA(
        ...
        const float2* __restrict__ pixel_velocities,
        int W, int H,
        ...
        const int n_blur_samples,
        const float exposure_time,
        const float* __restrict__ dL_dpixels,
        const float* __restrict__ dL_dpixel_depths,
        const float* __restrict__ dL_dalphas,
        float2* __restrict__ dL_dpixel_vel,
    ...
    __shared__ float2 collected_pixel_vels[BLOCK_SIZE];
    ...
    for (int32_t blur_i = 0; blur_i < n_blur_samples; ++blur_i) {
        const float blur_rel = ((n_blur_samples > 1) ? (float(blur_i) / (n_blur_samples - 1) - 0.5f) * exposure_time : 0.0f);
        ...
        const float T_final = inside ? final_Ts[blur_sample_id] : 0;
        float T = T_final;
        const float T_final_mean = T_final * blur_avg_factor;

        // We start from the back. The ID of the last contributing
        // Gaussian is known from each pixel from the forward.
        uint32_t contributor = toDo;
        const int last_contributor = inside ? n_contrib[blur_sample_id] : 0;
        ...
        for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE) {
            ...
            if (range.x + progress < range.y) {
                ...
                collected_pixel_vels[block.thread_rank()] = pixel_velocities[coll_id];
            }
            block.sync();
            ...
            for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++) {
                ...
                const float2 pix_vel = collected_pixel_vels[j];
                const float2 delta = {xy.x + blur_rel * pix_vel.x - pixf.x, xy.y  + blur_rel * pix_vel.y - pixf.y};
                const float4 con_o = collected_conic_opacity[j];
                const float power = -0.5f * (con_o.x * delta.x * delta.x + con_o.z * delta.y * delta.y) - con_o.y * delta.x * delta.y;
                if (power > 0.0f) {
                    continue;
                }
                ...
                const float dchannel_dcolor = alpha * Tmean;
                const float dpixel_depth_ddepth = alpha * Tmean;
                ...
                dL_dopa += (-T_final_mean / (1.f - alpha)) * bg_dot_dpixel;

                // Helpful reusable temporary variables
                const float dL_dG = con_o.w * dL_dopa;
                const float gdx = G * delta.x;
                const float gdy = G * delta.y;
                const float dG_ddelx = -gdx * con_o.x - gdy * con_o.y;
                const float dG_ddely = -gdy * con_o.z - gdx * con_o.y;

                // Update gradients w.r.t. 2D mean position of the Gaussian
                const float dL_dmean2D_cur_x = dL_dG * dG_ddelx * ddelx_dx;
                const float dL_dmean2D_cur_y = dL_dG * dG_ddely * ddely_dy;
                atomicAdd(&dL_dmean2D[global_id].x, dL_dmean2D_cur_x);
                atomicAdd(&dL_dmean2D[global_id].y, dL_dmean2D_cur_y);

                atomicAdd(&dL_dpixel_vel[global_id].x, dL_dmean2D_cur_x * blur_rel);
                atomicAdd(&dL_dpixel_vel[global_id].y, dL_dmean2D_cur_y * blur_rel);

                // Update gradients w.r.t. 2D covariance (2x2 matrix, symmetric)
                atomicAdd(&dL_dconic2D[global_id].x, -0.5f * gdx * delta.x * dL_dG);
                atomicAdd(&dL_dconic2D[global_id].y, -0.5f * gdx * delta.y * dL_dG);
                atomicAdd(&dL_dconic2D[global_id].w, -0.5f * gdy * delta.y * dL_dG);
...
void BACKWARD::render(
    ...
    int W, int H,
    const int blur_samples,
    const float exposure_time,
    const float2* pixel_velocities,
